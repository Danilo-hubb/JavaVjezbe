...
class Auto {
	...
	public Auto(String markaAuta, int godisteAuta, float snagaMotora,
			boolean prodato, float kubikaza, boolean registrovan) {
	...
	public void setMarka(String markaAuta) {
		this.markaAuta = markaAuta;
	}
	...
	public void setRegisreovan(boolean registrovan) {
	...
	public double cijenaRegistracije() {
		if (!this.mozeSeRegistrovati()) return 0.0;
		return koeficijentZaGodiste(godisteAuta) * kubikaza * snagaMotora;
	}
	public String toString() {
		return String.format("Auto[marka=%s, godiste=%, snaga=%.1f, " +
	"kubikaza=%.1f, registrovan=%s, prodato=%s, cijenaReg=%.2f]", markaAuta, godisteAuta, snagaMotora, kubikaza, registrovan ? "da" : "ne" , prodato ? "da" : "ne", cijenaRegistracije());
...
public static double ukuonaRegistracija(ArrayList<Auto> auto) {
	double suma = 0;
	for (Auto a:auto) {
